...
export default function Projects() {
    const projects = [
        {n:1, projeceName:"(freelancing) building teams for private school",brife:"build a whole system for a private school in Microsoft business and set up teams for the class "}, 
        ...
        mx-auto items-center'>
         ...
         text-2xl '
            ...
            <div className=' relative w-full flex overflow-x-scroll 
             overflow-y-hidden snap-x snap-mandatory  scrollbar'>
                {projects.map(({projeceName,brife,i}) => (

                    <div key={projects.length}
                    projeceName={projeceName}
                    brife={brife}
                     className='snap-start w-screen flex-shrink-0 flex flex-col space-y-5 items-center justify-center p-20 md:p-40'>
                        ...
                        <div className='space-y-10 px-0 md:px-10 max-w-6xl'>
                            <h4 className='text-3xl font-semibold text-center'>
                                <span className='underline decoration-white/50'> {projeceName}</span>
                            </h4>
                            <p className='flex flex-row text-center'>{brife}</p>
                        </div>
                    </div>
                ))}
            </div>
