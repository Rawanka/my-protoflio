...
export default function Projects() {
    const projects = [
        {n:1, projeceName:"(freelancing) building teams for private school",brief:"build a whole system for a private school in Microsoft and set up teams for the class "}, 
        {n:2, projeceName:"Airbnb clone",brief:"buildeing Airbnb clone for learning porpese "}, 
        {n:3, projeceName:"Admin Dashborad App",brief:""},
        {n:4, projeceName:"build portfolio",brief:"building this portfolio by nextjs, tailwindcss and other liabrys"}
    ...
    return (
        ...
        md:text-left md:flex-row lg:text-left px-10 justify-evenly
         ...
         text-2xl '
            ...
            <div className=' relative w-full flex overflow-x-scroll 
             overflow-y-hidden snap-x snap-mandatory z-20 scrollbar'>
                {projects.map(({projeceName,brife,i}) => (

                    <div key={projects.length}
                    projeceName={projeceName}
                    brife={brife}
                     className='snap-start w-screen flex-shrink-0 flex flex-col space-y-5 items-center justify-center p-20 md:p-40 lg:p-40'>
                        ...
                        <div className='space-y-10 px-0 md:px-10 max-w-6xl text-center'>
                            <h4 className='text-3xl font-semibold text-center'>
                                <span className=' text-blue-200 decoration-blue/50 md:p-50'> {projeceName}</span>
                            </h4>
                            <p className='flex flex-row text-center p-5'>{brife}</p>
                        </div>
                    </div>
                ))}
            </div>
            <div className='w-full absolute top-[35%] bg-blue-700/10
