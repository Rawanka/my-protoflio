...
const info = {
  option1: [
    { title: 'Info 1', description: 'This is information for Option 1' },
    { title: 'Info 2', description: 'Additional information for Option 1' },
  ],
  option2: [
    { title: 'Info 3', description: 'This is information for Option 2' },
    { title: 'Info 4', description: 'Additional information for Option 2' },
  ],
  option3: [
    { title: 'Info 5', description: 'This is information for Option 3' },
    { title: 'Info 6', description: 'Additional information for Option 3' },
  ],
};
...
xl:space-y-0 mx-auto items-center'>
    ...
    text-gray-500 text-sm'>Select Skill </h2>
     <div className='absolute top-72 items-center justify-center h-full space-x-5 text-gray-400 text-xl'>
       <button
     ...
     <div className='w-full flex space-x-5 overflow-x-scroll scrollbar p-10 snap-x snap-mandatory'>
        {skills.map(({ id, skill, img }) => (
          <SkillCard key={id} skill={skill} img={img} showInfo={selectedOption === 'option3'} />
        ))}
      </div>
      <div className='absolute top-96 text-center w-full'>
        {selectedOption && info[selectedOption]?.map((item, index) => (
          <div key={index}>
            <h3>{item.title}</h3>
