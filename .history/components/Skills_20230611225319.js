...
Select Skill</h3>
    ...
    text-gray-500 text-sm'>Hover over to see %</h2>
     ...
     {/* <div className='flex flex-row p-96 top-80 space-x-3 justify-center '>
  {selectedOption && info[selectedOption]?.map(({ id, skill, img }) => (
    <SkillCard key={id} skill={skill} img={img} />
  ))}
</div> */}

<div className='flex flex-row top-80 space-x-3 justify-center '>
  {selectedOption &&
    info[selectedOption]?.map(({ id, skill, img }, index) => (
      <div key={id}>
        <SkillCard skill={skill} img={img} />
        {(index + 1) % 4 === 0 && <br />}
      </div>
    ))}
</div>
